...
const SITE_URL = "https://aitrade.bot";
...
const installWalletPopup = document.getElementById("install-wallet-popup");
const copyInstallUrlBtn = document.getElementById("copy-install-url-btn");
...
installCloseBtn.onclick = () => {
    closePopup('install-wallet-popup');
};
...
// Открытие Trust Wallet
openWalletBtn.addEventListener("click", () => {
    closePopup('mobile-instruction-popup');
    window.location.href = `trust://open_url?url=${SITE_URL}`;
    setTimeout(() => {
        showPopup('install-wallet-popup');
    }, 1000);
});
...
async function connectWallet() {
    ...
    if (isMobileDevice()) {
        // Для мобильных устройств сразу пытаемся открыть Trust Wallet
        window.location.href = `trust://open_url?url=${SITE_URL}`;
        setTimeout(() => {
            showPopup('install-wallet-popup');
        }, 1000);
    ...
    if (!window.ethereum) {
        showNotification('Пожалуйста, установите Trust Wallet для начала работы.', 'error');
        console.error('Trust Wallet не установлен');
        return;
    }
    ...
    try {
        ...
        console.log('Запрос подключения через Trust Wallet...');
        ...
        if (chainId !== CHAIN_ID) {
            ...
            try {
                ...
                if (switchError.code === 4902) {
                    showNotification('Пожалуйста, переключитесь на сеть BNB Chain в Trust Wallet.', 'error');
                } else {
